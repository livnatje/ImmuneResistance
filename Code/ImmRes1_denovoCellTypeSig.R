get.cell.type.sig<-function(r = NULL,subtype.flag = F){
  ...
  if(!subtype.flag){
    ...
    write.csv(list.2.mat(cell.sig),row.names = F,
...
get.all.t.subset.sig<-function(rF = NULL,r4 = NULL,r8 = NULL){
  ...
  if(is.null(rF)){rF<-readRDS("../Data/scData/Mel.all.data.QC.rds")}
  if(is.null(r4)){r4<-readRDS("../Data/scData/Mel.T.CD4.QC.rds")}
  if(is.null(r8)){r8<-readRDS("../Data/scData/Mel.T.CD8.QC.rds")}
...
get.t.cell.subset.sig<-function(rF,r,t.markers,T.type = "T.CD8"){
  ...
  for(i in names(t.markers)){L[,i]<-ifelse(B[,i],i,"")}
...
find.Tregs<-function(r){
  if(is.null(r)){r<-readRDS("../Data/scData/Mel.T.CD4.QC.rds")}
