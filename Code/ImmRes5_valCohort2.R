prd.aPD1<-function(r = NULL,res.sig,mal.genes,fileName = "tmp"){
  ...
  aPD1.val$ttest<-cbind.data.frame(CR.vs.PR.PD = apply.ttest(t(r$res[b1,]),r$response[b1]=="CR",ranksum.flag = F),
  ...
  aPD1.val$auc<-cbind.data.frame(CR.vs.PR.PD = get.auc.mat(r$res[b1,],r$response[b1]=="CR"),
...
barplot.sig.prf<-function(z,l,main,res.sig.names,p.type = "t-test"){
  ...
  l<-gsub("."," ",l,fixed = T)
  l<-capitalize(l)
  barplot(height = abs(z),col = ifelse(z>0,"lightblue","gray"),
...
aPD1.plot.km<-function(r,plot.sig,res.sig.names.down){
  ...
  lapply(l[b2], function(x1){
    par(mfrow=c(2,2),oma = c(0, 0, 3, 0))
    x<-paste0(x1,".up");km.plot3(r,r$res[,x],xlim = 2,main = x,qua = qua,direction = 1, ylab = "PFS",X = r$tme[,"T.CD8"])
    x<-paste0(x1,".down");km.plot3(r,r$res[,x],xlim = 2,main = x,qua = qua,direction = (-1), ylab = "PFS",X = r$tme[,"T.CD8"])
    x<-x1;km.plot3(r,r$res[,x],xlim = 2,main = x,qua = qua,direction = 1,ylab = "PFS",X = r$tme[,"T.CD8"])
  })
  
  par(mfrow=c(2,3),oma = c(0, 0, 3, 0))
  lapply(l[!b2],function(x){km.plot3(r,r$res[,x],main = x,X = r$tme[,"T.CD8"],xlim = 2,qua = 0.2,
                                     direction = ifelse(is.element(x,res.sig.names.down),-1,1),ylab = "PFS")})
}

prd.aPD1.simple<-function(r,res.sig){
  v<-get.sign.scores(r,res.sig)
  v1<-t(get.residuals(t(v),colSums(r$tpm>0)))
  v2<-t(get.residuals(t(v1),r$cc.scores))
  aPD1.val<-list(res.sig = res.sig,
                 scores = v, scores.norm = v1,scores.cc = v2)
  aPD1.val$cox<-cbind.data.frame(ori = cox.mat(t(v),r)[,"Zscore"],
                                 norm = cox.mat(t(v1),r)[,"Zscore"],
                                 cc = cox.mat(t(v2),r)[,"Zscore"])
  
  b1<-is.element(r$cb,c("non-CB","CB"))
  b2<-r$response!="na"&!is.na(r$response)&r$response!="MR"
  b3<-is.element(r$cbs,c("non-CB","CB>1yr"))
  f<-function(v){
    p<-cbind.data.frame(CR.vs.PR.PD = apply.ttest(t(v1[b1,]),r$response[b1]=="CR",ranksum.flag = F)[,3],
                        CR.vs.others = apply.ttest(t(v1[b2,]),r$response[b2]=="CR",ranksum.flag = F)[,3],
                        CB.vs.nonCB = apply.ttest(t(v1[b1,]),r$cb[b1]=="CB",ranksum.flag = F)[,3],
                        longCB.vs.nonCB = apply.ttest(t(v1[b3,]),r$cbs[b3]=="CB>1yr",ranksum.flag = F)[,3])
  }
  aPD1.val$ttest<-f(v1)
  aPD1.val$ttest.cc<-f(v2)
  
  return(aPD1.val)
}

aPD1.plot.simple<-function(r,v,X = NULL,main = "",alternative = "two.sided",subplot.flag = T){
  if(subplot.flag){
    par(mfrow=c(1,2),oma = c(0, 0, 3, 0))
  }
  
  km.plot3(r,v,xlim = 2,main = main,ylab = "PFS",X = X)
  b<-is.element(r$response,c("CR","PR","PD"))
  p1<-t.test.labels(v[b],r$response[b]=="CR",alternative = alternative)
  p2<-t.test.labels(v[b],r$response[b]!="PD",alternative = alternative)
  p<-format.pval.private(c(p1,p2))
  l<-r$response
  l[r$response=="CR"]<-"1.CR";l[r$response=="PR"]<-"2.PR";l[r$response=="PD"]<-"3.PD"
  boxplot.test(v[b],l[b],main = paste(main,p,sep = "\n"),t.test.flag = "none",
                   ylab = "OE")
}
