...
write.tableS3B_cell.subtype.signatures<-function(cell.sig = NULL){
  if(is.null(cell.sig)){load("../Results/CellTypes/cell.type.sig.full.RData")}
  cell.sig$T.CD8.NK<-NULL;cell.sig$lymphocyte<-NULL
  ...
  names(cell.sig)<-toupper(names(cell.sig));summary(cell.sig)
  m<-list.2.mat(cell.sig)
  print("Writing TableS3B: Denovo cell sub/type signatures.")
  write.csv(m,row.names = F,
...
write.tableS4A_resistance.signatures<-function(res.sig = NULL,CDK7.targets = NULL){
  if(is.null(CDK7.targets)){load("../Data/PublicData/CDK7.MYC.targets.RData")}
  ...
  f<-function(d){
    ...
    rownames(m)<-genes
    colnames(m)<-paste0(c("Exclusion","Exclusion seed",
                          "Post-treatment","Functional immune resistance")," (",d,")")
...
generate.fig1.S1FG<-function(r = NULL,type.OE = NULL){
  if(is.null(r)){r<-readRDS("../Data/scData/Mel.all.data.QC.rds")}
  if(is.null(type.OE)){type.OE<-readRDS("../Results/CellTypes/cell.type.OE.rds")}
  cd8<-r$cd["CD8A",]>0|r$cd["CD8B",]>0
  cd4<-r$cd["CD4",]>0
  markers<-cbind.data.frame(CD8 = ifelse(cd8&!cd4,"Positive","Negative"),
...
generate.fig5<-function(r.tcga = NULL,R = NULL,r.pd1 = NULL,aPD1.val = NULL){
  ...
  m<-cbind.data.frame(sig = c("res","ccf.res","resF","resF.minus.TIL"),
                      names = c("Immune resistance","Immune resistance\n(cell cycle residuals)",
