...
cdk46i.efficacy.analysis<-function(r,state,q.drug.sen = 0.1,q.drug.res = 0.5,q.res.state = 0.9){
  ...
  r$state<-state
  l<-cbind.data.frame(palbo = paste0("palbo.",f(r$cdk46.ic50$palbo)),
  ...
  M1 <- with(r0, glmer (y ~ stateB + (1 | types), family = binomial(link="logit"),
                        control=glmerControl(optimizer="Nelder_Mead",optCtrl=list(maxfun=20000))))
  M2 <- with(r0, glmer (y ~ state + (1 | types), family = binomial(link="logit"),
                        control=glmerControl(optimizer="Nelder_Mead",optCtrl=list(maxfun=20000))))
  Z<-rbind(discrete = summary(M1)$coef["stateBTRUE",c("Estimate","Pr(>|z|)")],
