...
test.seed.sig<-function(r,results,no.itr = 100){
  ...
  scores.att<-round(get.OE.sc(r,rnd.seed.att,num.rounds = 100),2);scores.att[,1]<-results$sc.seed.scores[,"att"]
  scores.exc<-round(get.OE.sc(r,rnd.seed.exc,num.rounds = 100),2);scores.exc[,1]<-results$sc.seed.scores[,"exc"]
  ...
  raw.scores.att<-get.semi.random.OE(r = r,genes.dist.q = r$genes.dist.q,
                                     b.sign = is.element(r$genes,results$seed.sig$att),
                                     num.rounds = no.itr,full.flag = T)
  raw.scores.exc<-get.semi.random.OE(r = r,genes.dist.q = r$genes.dist.q,
...
get.post.trt.sig<-function(r = NULL,subs.trt = NULL){
  ...
  trt.de$sig<-list(trt.up = sort(intersect(sig.subsmp$up,trt.de$sig$up)),
...
get.res.program<-function(exc.sig = NULL,trt.sig = NULL,fnc.sig = NULL){
  ...
  res.sig<-c(exc.sig,trt.sig,fnc.sig,
             list(resu.up = c(trt.sig$trt.up,exc.sig$exc.up),
...
find.DEG<-function(r,main = "",mainP = "treated",batch = NULL,no.elm = 200){
  ...
  de$sig<-get.scde.ttest.sig(de,no.elm = no.elm,scde.c = (-1.96),
